...
for Section in range(1): #Input parameters
    ...
    NI=100 #Number of intervals in the model
    FDME0=0 #[mol/s]
    FMetOH0=0 #[mol/s]
    FH2O0=0 #[mol/s]
    FH20=3 #[mol/s]
...
for Section in range(1): #Set variables
    ...
    T=m.MV(value=500, lb = 400, ub = 700) #Optimize - Isothermal
    T.STATUS = 1
    T.DCOST = 3
    P=m.MV(50, lb=1, ub = 80) #Optimize
    P.STATUS = 1
    P.DCOST = 3
...
final = np.zeros_like(m.time); final[-1] = 1
final = m.Param(final)
#Set solver options
m.Maximize(FDME*final*10000)
m.options.IMODE=6
